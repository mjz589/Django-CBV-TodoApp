...
@pytest.fixture
def common_user():
    user = User.objects.create_user(email='testaccount@test.com', password='test/!1234', is_verified=False)
    return user
...
class TestTaskApi():
    ...
    def test_post_reset_password_request_response_200_status(self, api_client, common_user):
    ...
    def test_put_reset_password_token_200_status(self, api_client, create_token):
        ...
        data={
            'password1': '@my/1234567',
            'password2': '@my/1234567'
        }
        response = api_client.put(url, data=data)
        assert response.status_code == 200
